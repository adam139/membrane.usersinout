...
from membrane.usersinout.config import CSV_HEADER, MEMBER_PROPERTIES
...
#        pdb.set_trace()
    ...
    def _getUsersInfos(self):
        ...
        for user in acl.searchUsers():
            if  user['pluginid'] == 'membrane_users':                
    ...
    def _getUserData(self,userId):
        ...
        memberbrains = catalog(object_provides=IMember.__identifier__, 
                                email=userId)
        memberobj = memberbrains[0].getObject()
                                
        
        if memberobj is not None:
            for p in MEMBER_PROPERTIES: # dexterity member properties
                props.append(getattr(memberobj,p))
